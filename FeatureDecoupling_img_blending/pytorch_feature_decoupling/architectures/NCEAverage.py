...
class NCEFunction(Function):
    ...
    def forward(self, x, y, memory, idx, params):
        ...
        weight = weight.view(batchSize, K+1, inputSize).clone()

        # inner product
        x = x.view(batchSize, inputSize, 1).clone()
        out = torch.bmm(weight, x.data)
        out.div_(T).exp_() # batchSize * self.K+1
        x=x.view(batchSize, inputSize).clone()
        ...
        out= out.div_(Z).view(batchSize, K+1).clone()
    ...
    def backward(self, gradOutput):
        ...
        gradOutput = gradOutput.view(batchSize, 1, K+1).clone()
        
        # gradient of linear
        gradInput = torch.bmm(gradOutput.data, weight)
        gradInput=gradInput.view_as(x).clone()

        # update the non-parametric data
        weight_pos = weight.select(1, 0).view_as(x).clone()
