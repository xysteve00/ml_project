...
if __name__ == '__main__':
    ...
    net = utils_mult.get_model(args['<model>'])
    # print(net)
    ...
    if len(img_names) > 1000:
        img_order = np.random.choice(len(img_names),1000,replace=False)
    ...
    if(eval(args['--data_dep'])):
        batch_size = 1
        uap = utils_mult.universal_perturbation_data_dependant(img_names, ntar_img_names, net, blend_img_path, encoder,sub_model_type, xi=xi, delta=delta, max_iter_uni =max_iter_uni,
                                                          p=p, num_classes=num_classes, overshoot=overshoot, 
                                                          max_iter_df=max_iter_df,init_batch_size = batch_size,t_p = t_p)
    else:
        batch_size = int(args['--batch_size'])
        uap = utils_mult.universal_perturbation_data_independant(img_names, net,delta=delta, max_iter_uni = max_iter_uni, xi=xi,
                                                            p=p, num_classes=num_classes, overshoot=overshoot,
