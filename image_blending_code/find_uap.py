...
if __name__ == '__main__':
    ...
    np.random.seed(2)
    if len(img_names) > 900:
        img_order = np.random.choice(len(img_names),200,replace=False)
        img_order_t = np.random.choice(len(img_names),100,replace=False)
        img_names_sub = [img_names[img_order[i]] for i in range(img_order.shape[0])]
        ntar_img_names_sub = [ntar_img_names[img_order_t[i]] for i in range(img_order_t.shape[0])]
