...
testset = './data/sub_v1/scrap_test_target_105_n01882714.txt'
trainset = './data/sub_v1/scrap_train_target_105_n01882714.txt'
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=args.epochs):
    ...
    torch.save(model.state_dict(), 'saved_models/raw/n01882714/aug/model_{}_{}_{}_{}_{}_{}.pth'.format(args.tar_model, args.sub_model, epoch,best_acc,running_loss,os.path.basename(trainset).split('.')[0]))
...
model_name = 'saved_models/raw/n01882714/model_{}_{}_{}_{}_{}_{}.pth'.format(args.tar_model, args.sub_model,'v1',np.max(val),np.min(loss),os.path.basename(trainset).split('.')[0])
print(loss)
print(val)
