...
def hard_label_img_search(name_ori,ntar_name, model, tf, blend_img_path):
    ...
    img_to_save = []
    img_to_save_blend = []
    while abs(alpha_min - alpha_max)> 0.0001 and itr < 9:
        ...
        itr = itr + 1
    if bld_img_name_list and bld_img_score:
        iimg = F.interpolate(img_to_save_blend[-1],size=256)
        save_image(iimg,bld_img_name_list[-1])
        print('%s was saved|score: %f' % (bld_img_name_list[-1],bld_img_score[-1]))
        rv = [bld_img_name_list[-1],bld_img_score[-1],img_to_save_blend[-1]]
    if len(img_to_save):
...
def hard_label_encoder_img_search(name_ori,ntar_name, model,netG,
    ...
    if not opt:
        for alpha_mid in np.linspace(0.1,1.0,num=100):
            img_blend = img_blending_encoder(x_img_tensor.cuda(), y_img_tensor.cuda(), netG, alpha_mid)
            with torch.no_grad():
                logit_blend = model(img_blend.cuda()).cpu().data.numpy()
                blend_labels = np.argmax(logit_blend,1).astype(int)[0]
    ...
    for i in range(len(score_min)):
        ...
        img_name = os.path.join(blend_img_path,"example_blend_" +
                os.path.basename(ntar_name).split('.')[-2][-1:-3] +'_' + \
                        os.path.basename(name_ori).split('.')[-2][-1:-3]+'_' + \
                                str(true_labels) +'_'+
        ...
        rv.append((img_name,lab_min[i],score_min[i]))
    if len(img_to_save):
        img_blend_all = os.path.join(blend_img_path,"example_blend_" + \
            os.path.basename(ntar_name).split('.')[-2][-1] + \
            os.path.basename(name_ori).split('.')[-2].split('_')[-1]+'_' + \
...
def img_merge(im_orig,ntar_name, model, tf, alpha, blend_img_path, \
    ...
    img_blend = gradientBlend(Image.open(im_orig), Image.open(ntar_name), alpha)
...
def gradientBlend(target, source, blend):
    ...
    for x in range(0, source.shape[1]):
        ...
        for y in range(0, source.shape[0]):
            ...
            img_bld[x,y,:] = targetPixel*tarBlend + sourcePixel*srcBlend
...
def universal_perturbation_data_dependant(data_list, ntar_list, model, blend_img_path, encoder, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    for k in range(0, num_batches):
        ...
        for j, ntar_name in enumerate(ntar_list):
            ...
            #itr, img_to_save, img_to_save_blend,rv = hard_label_img_search(ntar_name,name_ori, model, tf, blend_img_path)
            ...
            for (lab,score,alpha) in rv:
                lab_list.append(lab)
                score_list.append(score)
                alpha_list.append(alpha)

            for (lab,score,alpha) in rv1:
                lab_list.append(lab)
                score_list.append(score)
                alpha_list.append(alpha)
