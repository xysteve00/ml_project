...
def train_model_i(model, criterion, optimizer, scheduler,dataloaders,dataset_sizes,trainset,testset, num_epochs=20):
    ...
    torch.save(model.state_dict(), 'saved_models/v3/pretrain/model_{}_{}_{}_{}_{}_{}.pth'.format(config.tar_model, config.sub_model, epoch,best_acc,running_loss,os.path.basename(trainset).split('.')[0]))
    return model
