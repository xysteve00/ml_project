...
def data_gen_target(img_test_final,target_list):
    ...
    img_tnindex = np.random.choice(len(img_test_nontarget_tot),1000,replace=False)
...
img_location = '/home/xiaoyu/home/data/images/train'
...
for ifolder in sub_folder:
    tmp_path = glob.glob(os.path.join(ifolder,'*'))
    for iifolder in tmp_path:
        img_file.extend(glob.glob(os.path.join(iifolder,'*')))
    print(len(img_file))
...
print(len(re_img_file))
print(len(img_test_final))
#with open('tiny_test.txt','w+') as f:
#    f.writelines(tar_t)

#with open('tiny_train.txt','w+') as f:
...
with open('place_test.txt','w+') as f:
    f.writelines(img_test_all_name)

with open('place_train.txt','w+') as f:
    f.writelines(img_train_all_name)

#with open('./data/sub_v1/scrap_test_target_105_n01882714_v2.txt','w+') as f:
#    f.writelines(img_test_all_name)

#with open('./data/sub_v1/scrap_train_target_105_n01882714_v2.txt','w+') as f:
#    f.writelines(img_train_all_name)
