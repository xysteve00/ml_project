...
class GeneratorResnet(nn.Module):
    ...
    def forward(self, input, embedding=None, encode_opt=True):
        x = self.block1(input)
        x = self.block2(x)
        x = self.block3(x)
        x = self.resblock1(x)
        ...
        if self.data_dim == 'high':
            ...
            # x = self.resblock9(x)
        x_emb = x
        x = x_emb if encode_opt else embedding
        x = self.upsampl1(x)
        x = self.upsampl2(x)
        x = self.blockf(x)
        if self.inception:
            x = self.crop(x)
        if encode_opt:
            return (torch.tanh(x) + 1) / 2, x_emb      
        return (torch.tanh(x) + 1) / 2, embedding # Output range [0 1]
