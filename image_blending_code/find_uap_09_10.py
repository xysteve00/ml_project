...
Usage:
  find_uap.py <model> <im_path> <im_path_ntar> <im_list> <ntar_list> <blend_img_path> [options]
  find_uap.py (-h | --help)
  find_uap.py --version
...
if __name__ == '__main__':
    ...
    net = utils_09_10.get_model(args['<model>'])
    # print(net)
    ...
    if len(img_names) > 1000:
        img_order = np.random.choice(len(img_names),100000,replace=False)
    ...
    if(eval(args['--data_dep'])):
        batch_size = 1
        uap = utils_09_10.universal_perturbation_data_dependant(img_names, ntar_img_names, net, blend_img_path, encoder,encoder1, xi=xi, delta=delta, max_iter_uni =max_iter_uni,
