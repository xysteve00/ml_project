...
def universal_perturbation_data_dependant(data_list, test_list, model, blend_img_path, encoder,encoder1, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    if not for_train:
        ...
        net = load_model('res18',state_dict=None, for_train=True)

    ########################
    ##target  model accu
    ###########################
    nl_test_dict,acu = model_predict(model, test_list,tf)
    #pickle.dump(nl_test_dict,open('./meta/nldict_test_target_v33.pickle','wb+'))
    ...
    print(len(tar_train))
    print(len(nontar))
    t_tar = copy.deepcopy(tar_train)
    t_ntar = []
    count_q = 0
    num_t = 10
    for t in range(num_t*10):
        print('Iteration: %d ' % t)
        print(len(tar_train))
        mix_len = min(600,int(len(t_tar)))
        ...
        if len(qnontar) >= len(itar):
            ...
            nontar_sub_mix = np.random.choice(len(nontar),len(num),replace=False)
