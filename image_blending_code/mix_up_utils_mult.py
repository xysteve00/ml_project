...
def get_accuracy(labels, pred):
    ...
    print(len(np.array(pred)))
    return np.mean(np.array(labels)==np.array(pred))
        ...
        for k in range(0, num_batches):
            ...
            for i,name in enumerate(data_inp):
                try:
                    ...
                    print('Bad file:', name,flush=True) # print out the names of corrupt files
                else:            
                    ...
                    if true_labels[0] ==  target:
                        corrections +=1 
                        pred_convert_list.append(0)
                    else:
                        pred_convert_list.append(1)
        acu = corrections / len(data_list)
        #print('input and pred len from model prediction')
        #print('recall:  %f'%(acu))
        name_lbl_dict = dict(zip(name_list,pred_list))
    return name_lbl_dict,pred_convert_list
...
def load_model(model_type, state_dict=None, for_train=False):
    if model_type == 'res18':
        ...
        netD.fc = nn.Linear(feature_size, 11)
    elif model_type == 'densenet':
        netD = torchvision.models.densenet121(pretrained=False)
        netD.classifier = nn.Linear(1024,11)
    elif model_type == 'mobilenet':
        netD = geffnet.create_model('mobilenetv3_rw', pretrained=False)
        netD.classifier = nn.Linear(in_features=1280, out_features=11, bias=True)
    if state_dict:
...
def label_stats(nld,target):
    ...
    for (key, value) in nld.items():
        if value in target:
            tar_list.append(key)
            tar_label.append(value)
            #print((key,value))
        else:
            nta_list.append(key)
    return tar_list, tar_label,nta_list
...
def hard_label_gen(tar_sub,nontar_mix, model, tf, blend_img_path,target_list=[895]):
    model = model.eval()
    with torch.no_grad():
        img_data = []
        score_list = []
        ...
        for k,name_img in enumerate(tar_sub):
            if 'home' in name_img.split('/'):
                name = os.path.relpath(name_img,'/home/ngoc/data2/P2')
            else:
                name = name_img
            cur_img = torch.zeros(1,3,224,224)
            bld_img = torch.zeros(1,3,224,224)
            ...
            num_iter = 0
            num_iter1 = 0 
            print('target image: %s  %d   %f' % (name_ori,true_labels[0], score_true))
            #for j in range(k+1,len(data_list)):
            for j, ntar_name_img in enumerate(nontar_mix):
                itr = 0
                if 'home' in ntar_name_img.split('/'):
                    ntar_name = os.path.relpath(ntar_name_img,'/home/ngoc/data2/P2')
                else:
                    ntar_name = ntar_name_img
                meta_name = name_ori
