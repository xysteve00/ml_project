...
def train_model_i(model, criterion, optimizer, scheduler,dataloaders,dataset_sizes,trainset,testset, num_epochs=20):
    ...
    torch.save(model.state_dict(), os.path.join(config.save_model,'model_{}_{}_{}_{}_{}_{}.pth'.format(config.tar_model, config.sub_model, epoch,best_acc,running_loss,os.path.basename(trainset).split('.')[0])))
    return model
                                                                                            

def model_update(model,trainset, testset, batch_size=config.batch_size, device=config.device,lr=config.lr,
                 tar_model=config.tar_model, sub_model=config.sub_model):
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=config.batch_size, pin_memory=True)
    test_data = MyDataset(txt=testset, transform=test_transform)
    test_loader = DataLoader(dataset=test_data, batch_size=config.batch_size, pin_memory=True)
