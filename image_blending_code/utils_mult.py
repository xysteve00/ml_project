...
from mix_up_utils_mult import *
from data_prcs_utils_mult  import *
import pickle
import copy
from run_exper_mult import *
import shutil
...
def universal_perturbation_data_dependant(data_list, test_list, model, blend_img_path, encoder,sub_model_type, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    blend_img_path = config_mult.img_path
    ...
    ###########################
    sub_model_type = config_mult.sub_model
    print('sub_model_type: %s' % sub_model_type)
    print('save models in %s' % config_mult.save_model)
    for_train = True
    if not for_train:
        ...
        if sub_model_type == 'res18':
            model_dict = './saved_models/v3/n02749479/model_res50_res18_19_0.87875_251.07966709136963_img_train_5.pth'
        elif sub_model_type == 'densenet':
            model_dict = './saved_models/v3/v3/final/model_res50_densenet_19_0.9112291350531108_imagenet_0.92_img_train_8.pth'

        net = load_model(sub_model_type,state_dict=None, for_train=True)
    ...
    #    print('%s  %d'%(key,value))
    #sys.exit()
    target_list = config_mult.target_list
    print('target classes: %s' % target_list)
    ...
    np.random.seed(10)
    tar,tar_label,nontar = label_stats(name_lbl_dict,config_mult.target_list)
    test_tar,test_tar_label,_ = label_stats(nl_test_dict,config_mult.target_list)
    #test_nontar_index = np.random.choice(len(test_nontar0),int(len(test_tar)/9),replace=False)
    #test_nontar = [test_nontar0[test_nontar_index[i]] for i in range(test_nontar_index.shape[0])]
    print('test target: %d ' % (len(test_tar)))
    ###################
    ### test data#####
    ######################
    save_data = './data/sub_v3/'
    test_data = 'test_mult_final_'+str(config_mult.cls_num)+'_'+config_mult.tar_model+'_'+config_mult.sub_model+'_'+config_mult.job_name+'.txt'
    #test_data = './data/hl_tarm.txt'
    img_test = merge_img_listi(test_tar, test_tar_label)
    print('img all: %d'% len(img_test))
    ...
    t_tar_label = copy.deepcopy(tar_label)
    t_ntar = []
    count_q = 0
    num_t = int(len(tar)/12)
    for t in range(100):
        print('Iteration: %d ' % t)
        mix_len = min(200,int(len(t_tar)))
        ...
        qtar,qtar_label,qnontar = label_stats(qdata_dict,target_list)
        ...
        itar = t_tar + qtar
        itar_label = t_tar_label + qtar_label
        #itar = t_tar
        ...
        img_all = merge_img_listi(itar,itar_label)
        print(len(img_all))
        ...
        _ = data_test(img_all,save_data,trainset)
        net_cur = model_update(net,os.path.join(save_data, trainset),os.path.join(save_data, test_data))
        t_tar, t_tar_label = tar_non_tar_update(t_tar,t_tar_label, qtar,qtar_label)
        if len(t_tar) > 20000:
            t_tar = copy.deepcopy(tar)
            t_tar_label = copy.deepcopy(tar_label)
