...
def merge_img_listi(tar,tar_label):
    img_data = []
    random.seed(0)
    print('merge_img_listi before: %d' % len(tar))
    for item_tar,label in zip(tar,tar_label):
        if label == 403:
            img_data.append((item_tar,0,403))
        elif label == 413:
            img_data.append((item_tar,1,413))
        elif label == 471:
            img_data.append((item_tar,2,471))
        elif label == 652:
            img_data.append((item_tar,3,652))
        elif label == 833:
            img_data.append((item_tar,4,833))
        elif label == 847:
            img_data.append((item_tar,5,847))
        elif label == 867:
            img_data.append((item_tar,6,867))
        elif label == 734:
            img_data.append((item_tar,7,734))
        elif label == 895:
            img_data.append((item_tar,8,895))
        elif label == 366:
            img_data.append((item_tar,9,366))
        elif label == 105:
            img_data.append((item_tar,10,105))

        #print(item_tar.strip().split('/')[3])
        #if item_tar.strip().split('/')[3] == 'n02687172':
        #    img_data.append((item_tar,0,403))
        #elif item_tar.strip().split('/')[3] == 'n02749479':
        #    img_data.append((item_tar,1,413))
        #elif item_tar.strip().split('/')[3] == 'n02950826':
        #    img_data.append((item_tar,2,471))
        #elif item_tar.strip().split('/')[3] == 'n03763968':
        #    img_data.append((item_tar,3,652))
        #elif item_tar.strip().split('/')[3] == 'n04347754':
        #    img_data.append((item_tar,4,833))
        #elif item_tar.strip().split('/')[3] == 'n04389033':
        #    img_data.append((item_tar,5,847))
        #elif item_tar.strip().split('/')[3] == 'n04467665':
        #    img_data.append((item_tar,6,867))
        #elif item_tar.strip().split('/')[3] == 'n03977966':
        #    img_data.append((item_tar,7,734))
        #elif item_tar.strip().split('/')[3] == 'n04552348':
        #    img_data.append((item_tar,8,895))
        #elif item_tar.strip().split('/')[3] == 'n02480855':
        #    img_data.append((item_tar,9,366))
        #elif item_tar.strip().split('/')[3] == 'n01882714':
        #    img_data.append((item_tar,10,105))
        #else:
        #    print(item_tar.strip().split('/'))

    #sys.exit()
    #for item_nt in nontar:
    #    img_data.append((item_nt,1,1))
    print('merge_img_listi: %d' % len(img_data))
    random.shuffle(img_data)
    return img_data
...
def tar_non_tar_update(t_tar,t_tar_label, qtar,qtar_label):
    max_num = 100000 
    ...
    keep_label = [qtar_label[keep_index[i]] for i in range(keep_index.shape[0])]
    t_tar.extend(keep_sample)
    t_tar_label.extend(keep_label)
    #nontar.extend(np.random.choice(len(qtar),num_tar,replace=False))
    return t_tar,t_tar_label
