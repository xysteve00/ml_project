...
from image_mix_up import model_predict, load_model, get_embeddings,get_blending_pairs,label_stats,pca_plot,embedding_target
from data_preprcs import *
import pickle
...
def hard_label_img_search(name_ori,ntar_name, model, tf, blend_img_path,target_list,opt=False):
    ...
    if true_labels not in target_list:
        print('checking consistent')
        print(name_ori)
        print(true_labels)
    ...
    if not opt:
        for alpha_mid in np.linspace(0.0,1.0,num=24):
            img_blend,blend_labels,score_img_bld = img_merge(name_ori,ntar_name, model, tf, alpha_mid,
                blend_img_path,true_labels, score_true)
            img_to_save.append(img_blend)
            lab_list.append(blend_labels)
    ...
    for i in range(len(score_min)):
        ...
        if lab_min[i] == true_labels and true_labels in target_list:
            #print('true label: %d   %d'%(lab_min[i],true_labels))
...
def img_blending_encoder_br(x_img_tensor, y_img_tensor, netG,x, y, k):
    ...
    netG.eval()
    mask = torch.ones(1,osize,56,56).cuda()
    #mask[:,:,x:(x+k),y:(y+k)] = 0
    mask[:,x:y:k,:,:] = 0
    with torch.no_grad():
        _,x_emb = netG(x_img_tensor)
...
def universal_perturbation_data_dependant(data_list, ntar_list, model, blend_img_path, encoder,encoder1, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    """
    ...
    print('total images: %d'% num_images)
    iter_count = 0
    netG = UNet(3,3)
    ...
    ###########################
    #net = load_model('res18', './saved_models/raw/model_res50_res18_29_1.0_0.0_rl.pth')
    #_,sub_acu = model_predict(net, data_list,tf,target=0)
    ...
    pickle.dump(name_lbl_dict, open('./meta/nldict_all.pickle','wb+'))
    #sys.exit()
    ...
    #pickle.dump(tar,open('./meta/tar.pickle','wb+'))
    #pickle.dump(nontar, open('./meta/ntar.pickle','wb+'))
    #tar = pickle.load(open('./meta/tar.pickle','rb'))
    #nontar = pickle.load(open('./meta/ntar.pickle','rb'))
    print('tar nontar length')
    print(len(tar))
    print(len(nontar))
    tar_sub,nontar_sub, nontar_mix = split_ntlist(tar, nontar)

    for i, name in enumerate(tar):
        #print('%d   %s'%(i,name))
        if name in tar_sub:
            print(name,name_lbl_dict[name])
    ...
    for k,name in enumerate(tar_sub):
        ...
        num_iter = 0
        num_iter1 = 0 
        print('name_ori(%d): %s   %d   %f' % (k, name_ori,true_labels[0], score_true))
        ...
        for j, ntar_name in enumerate(nontar_mix):
            ...
            itr, img_to_save, img_to_save_blend,rv,hltar = hard_label_img_search(name_ori,ntar_name, model, tf, blend_img_path,target_list)
    ...
    pickle.dump(img_data,open('./meta/sthd_100_1000.pickle','wb+'))
    #img_data = pickle.load(open('./meta/sthd_50_400.pickle','rb'))
    print('img size')
    print(len(img_data))
    print(len(tar))
    print(len(nontar_sub))
    img_all = merge_img_list(img_data,tar,nontar_sub,name_lbl_dict)
    print('total length')
    print(len(img_all))
    _,_ = data_split(img_all,'./data/','img_train_100_1000.txt','img_val_100_1000.txt','img_test_100_1000.txt')
