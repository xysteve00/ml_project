...
def hard_label_img_search(name_ori,ntar_name, model, tf, blend_img_path,opt=False):
    ...
    if not opt:
        for alpha_mid in np.linspace(0.0,1.0,num=16):
            img_blend,blend_labels,score_img_bld = img_merge(name_ori,ntar_name, model, tf, alpha_mid,
                blend_img_path,true_labels, score_true)
            img_to_save.append(img_blend)
            lab_list.append(blend_labels)
...
def universal_perturbation_data_dependant(data_list, ntar_list, model, blend_img_path, encoder,encoder1, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    #pickle.dump(name_lbl_dict, open('./meta/nldict_100000.pickle','wb+'))
    #sys.exit()
    ...
    #pickle.dump(img_data,open('./meta/sthd.pickle','wb+'))
    print('img size')
    print(len(img_data))
    print(len(tar))
    print(len(nontar_sub))
    ...
    print('total length')
    print(len(img_all))
    #_,_ = data_split(img_all,'./data/','img_train_ft2.txt','img_val_ft2.txt','img_test_ft2.txt')
