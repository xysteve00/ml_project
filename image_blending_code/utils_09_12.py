...
def universal_perturbation_data_dependant(data_list, test_list, model, blend_img_path, encoder,encoder1, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    if not for_train:
        ...
        model_dict = './saved_models/raw/model_res50_res18_29_0.7468750000000001_333.7854356765747_img_train_raw.pth'
        net = load_model('res18',model_dict, for_train=True)
    ...
    pickle.dump(name_lbl_dict, open('./meta/nldict_train_v32.pickle','wb+'))
    pickle.dump(nl_test_dict,open('./meta/nldict_test_target_v32.pickle','rb'))
    ...
    test_nontar_index = np.random.choice(len(nontar),len(test_tar),replace=False)
    test_nontar = [nontar[test_nontar_index[i]] for i in range(test_nontar_index.shape[0])]
    print('test target: %d  test nontarget: %d:nontar0  %d'%(len(test_tar), len(test_nontar),len(test_nontar0)))
    ###################
    ### test data#####
    ######################
    save_data = './data/v1/'
    test_data = 'test_target.txt'
    ...
    for t in range(num_t*2):
        print('Iteration: %d ' % t)
        mix_len = min(1000,int(len(t_tar)))
        _,nontar_sub, nontar_mix = split_ntlist(tar_train, nontar, num_mix=mix_len)
        start = math.floor(len(tar_train)/num_t*(t%num_t)) 
        end = min(len(tar_train), math.floor(len(tar_train)/num_t*(t%num_t+1)))
        print('start')
        ...
        img_data =  hard_label_gen(tar_sub,nontar_mix, net, tf, blend_img_path,netG,target_list=[0])
        #query target model
        ...
        itar = t_tar + qtar
        nontar_sub_mix = np.random.choice(len(nontar),len(itar),replace=False)
        inontar = [nontar[nontar_sub_mix[i]] for i in range(nontar_sub_mix.shape[0])]
        print('tar_train: %d t_tar: %d qtar: %d itar: %d inontar %d'%(len(tar_train),len(t_tar),len(qtar),len(itar),len(inontar)))
