...
parser.add_argument('--pretrain', type=str, default=None, help='fune-ture pretrained model')
...
trainset = './data/img_train_ft7.txt'
#testset  = './data/hl_tarm.txt'
#testset = './data/v1/test_target.txt'
testset = './data/img_val_ft7.txt'
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=args.epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, (inputs,labels) in enumerate(dataloaders[phase]):
                ...
                # statistics
                if i % 100 == 9:
                    print('Epoch: {0} \t Batch: {1} \t loss: {2:.5f}'.format(epoch, i, running_loss / 100),flush=True)
                    #print(preds)
                    #print(labels)
    ...
    torch.save(model.state_dict(), 'saved_models/hl_from_tarm/model_{}_{}_{}_{}_{}_{}_pretrain_v4.pth'.format(args.tar_model, args.sub_model, epoch,best_acc,running_loss,os.path.basename(trainset).split('.')[0]))
    return model
...
netD = train_model(netD, criterion, optimizer, scheduler, num_epochs=30)
