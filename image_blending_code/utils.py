...
def universal_perturbation_data_dependant(data_list, ntar_list, model, blend_img_path, encoder,encoder1, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, num_classes=10, overshoot=0.02, max_iter_df=10,init_batch_size = 1,t_p = 0.2):
    ...
    cls_set = set()
    ##classifiy images
    #name_lbl_dict = model_predict(model, data_list,tf)
    #target_list = [895]
    ...
    for k,name in enumerate(tar):
        ...
        for j, ntar_name in enumerate(nontar):
            itr = 0
            meta_name = name_ori
